import { EnumFaltaLocal } from './enumerators';
...
import { Falta } from './fault';
import { CurtoCircuito, TensaoPosFalta, CorrenteFalta } from './short-circuit';
    ...
    toTable(field: string, withHeader = true): any[] {
        ...
        if (field === 'fluxos') {
            ...
            this[field].forEach(row => {
                let porcentagem = 1;
                if (this.hasFalta() && this.falta.linha) {
                    if (this.falta.linha.id_linha === row.id_linha) {
                        porcentagem = this.falta.porcentagem;
                    }
                }
                table.push(row.toArray(porcentagem));
            });
